...
class OrchestratorCLI:
    ...
    def run_task(self, user_input):
        ...
        progress_thread = threading.Thread(
            target=self.progress_monitor, daemon=True)
    ...
    def interactive_mode(self):
        ...
        auto_save = self.orchestrator.config.get(
            'output', {}).get('auto_save', False)
        output_dir = self.orchestrator.config.get(
            'output', {}).get('directory', 'outputs')
        if auto_save:
...
def main():
    ...
    if args.list_models:
        ...
        for model_key, config in factory.get_available_models().items():
            ...
            print(
                f"    Recommended for: {', '.join(config['recommended_for'])}")
            print()
        return

    cli = OrchestratorCLI(agent_model=args.agent_model,
                          no_save=args.no_save, output_dir=args.output_dir)
